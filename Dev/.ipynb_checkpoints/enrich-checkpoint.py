...
def enrich(gene_list, uni_list,uniprot2input,pathway_sizes, Dict, ncHGT=False,FDR=.05, 
    ...
    if kegg == False:
        ...
        if ncHGT: 
            background_gene_list_size = len(utils.csv2dict(f'../data/kegg/wol2/{input_type}-to-reaction.map', sep = '\t')) #should adjust this when filtering is applied
...
def enrich_wrapper(filename, input_type, method = 'set', return_all = False, FDR=.05,fpath= '../test_data', display_gene_symbol = True, display_input = False, 
    ...
    if input_type in ['uniprot']:
        ...
        gene_list_converted, backend2input, not_converted = utils.convert_IDs(gene_list,input_type, backend_type = backend_type, enrich_against = enrich_against)
    #read in dictionary and the gocam sizes
    #x = pd.read_csv(gcs)
    #gocam_sizes = pd.Series(x.sizes.values,index=x.gocam)
    Dict = utils.csv2dict(id2g,sep = sep)
    pathways2members = utils.reverse_dict(Dict)
    sizes = []
    ...
    if display_gene_symbol == True:
        results[4]['shared entities in gocam'] = utils.backend2gene(results[4]['shared entities in gocam'])
