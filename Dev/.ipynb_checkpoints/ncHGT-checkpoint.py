...
def do_ncHGT(k,gc,M,N, input_type = '', kegg = False, backend_type = '', enrich_against = None):
    ...
    if kegg:
        ...
        if os.path.isfile(f'../data/kegg/wol2/{enrich_against}-to-{backend_type}.map') == False:
            utils.kegg_make_pathway_reaction(backend_type, enrich_against)
             
    setID2members = utils.csv2dict(setID2members, sep = sep)
    gocam2ID = utils.csv2dict(gocam2ID, sep = sep)
    ID2gocam = utils.csv2dict(ID2gocam, sep = sep)
